...
function pic_change()
{
    ...
    print 
            '<h3>Pic Change</h3>
<form enctype="multipart/form-data" action="preferences_c3.php?action=picchange2" method="POST">
...
function do_pic_change()
{
    ...
    $file_info = getimagesize($_FILES['uploadedfile']['tmp_name']);

    if(empty($file_info)) {
            echo 'Invalid Image.<br />
        	&gt; <a href="preferences_c3.php?action=picchange">Go Back</a>';
            die($h->endpage());
    }
    
    
    if(!in_array($file_info['mime'], $mimes)){
            echo 'Bad mime type.<br />
        	&gt; <a href="preferences_c3.php?action=picchange">Go Back</a>';
            die($h->endpage());
    }
